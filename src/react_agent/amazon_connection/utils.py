...
class AdaptiveRateLimiter:
    """Implements adaptive rate limiting for Amazon requests."""

    def __init__(self, initial_rpm=20, min_rpm=5, max_rpm=30):
        self.current_rpm = initial_rpm
        self.min_rpm = min_rpm
        self.max_rpm = max_rpm
        self.interval = 60 / self.current_rpm  # seconds between requests
        self.last_request_time = 0
        self.lock = asyncio.Lock()
        self.success_count = 0
        self.failure_count = 0
        self.adjustment_threshold = 5  # Number of requests before adjusting rate
    ...
    def report_failure(self, error_type=None):
        """Report a failed request to adjust rate."""
        self.failure_count += 1

        # If we detect blocking, immediately reduce rate
        if error_type and any(term in error_type.lower() for term in ["block", "captcha", "throttle"]):
            self._reduce_rate(immediate=True)
        else:
            self._adjust_rate_if_needed()

    def _adjust_rate_if_needed(self):
        """Adjust the request rate based on success/failure patterns."""
        total = self.success_count + self.failure_count

        if total >= self.adjustment_threshold:
            if self.failure_count == 0:
                # All successes, increase rate
                self._increase_rate()
            elif self.failure_count / total > 0.2:
                # More than 20% failures, reduce rate
                self._reduce_rate()
    ...
    def _increase_rate(self):
        ...
        if self.current_rpm < self.max_rpm:
            ...
            self.interval = 60 / self.current_rpm
            logger.info(f"Increasing request rate to {self.current_rpm:.1f} rpm")

    def _reduce_rate(self, immediate=False):
        ...
        self.interval = 60 / self.current_rpm
        logger.info(f"Reducing request rate to {self.current_rpm:.1f} rpm")
